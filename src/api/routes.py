...
def _rounded_aggregate_value(
    result: list[Dict[str, Any]], field: str, digits: int = 2
) -> float:
    """Return zero for an empty/non-numeric aggregate (common in sparse snapshots)."""
    value = result[0].get(field) if result else None
    return round(value, digits) if isinstance(value, (int, float)) else 0
...
def _are_sibling_orgs(login_a: str, login_b: str) -> bool:
    ...
    return bool(
        len(short) >= 4 and long_.startswith(short) and len(long_) / len(short) <= 3.0
    )
...
async def root():
    """Endpoint raíz."""
    return {"message": "TFG Backend API", "version": "1.0.0", "status": "running"}


@router.get("/health")
async def health_check():
    """Liveness and selected runtime providers; no credentials are reported."""
    return {"status": "healthy", **config.runtime_status(), "data_ready": db.is_ready()}


@router.get("/health/ready")
async def readiness_check(response: Response):
    """Readiness probe: validates the selected data backend without a GitHub call."""
    ready = db.is_connected()
    if not ready:
        response.status_code = 503
    return {"status": "ready" if ready else "unready", **config.runtime_status()}
...
async def get_stats():
    ...
    try:
        ...
        logger.info(
            f"📊 /stats calculado y cacheado: {repos_count} repos, {users_count} users, {orgs_count} orgs"
        )
...
async def get_dashboard_stats(
    force_refresh: bool = Query(
        default=False, description="Forzar recálculo ignorando caché"
    ),
    org: Optional[str] = Query(
        default=None, description="Filtrar por organización (login)"
    ),
    language: Optional[str] = Query(
        default=None, description="Filtrar por lenguaje de programación"
    ),
    repo: Optional[str] = Query(
        default=None, description="Filtrar por repositorio (full_name)"
    ),
    collab_type: Optional[str] = Query(
        default=None,
        description="Tipo de colaborador: 'contributors' (con commits), 'reviewers' (solo mencionables), 'all'",
    ),
    include_bots: bool = Query(
        default=False,
        description="Incluir cuentas de bots en el análisis de colaboradores",
    ),
    discipline: Optional[str] = Query(
        default=None,
        description="Filtrar colaboradores por disciplina (e.g. 'quantum_algorithms', 'hardware_engineering')",
    ),
):
    """
    ...
    try:
        ...
        has_filters = bool(
            org or language or repo or collab_type or include_bots or discipline
        )
        ...
        if not force_refresh and not has_filters:
            ...
            if cached_stats:
                ...
                logger.info(
                    "📊 Cache HIT - Dashboard stats servido desde caché permanente"
                )
        ...
        if has_filters:
            logger.info(
                f"📊 Calculando dashboard stats CON FILTROS: org={org}, language={language}, repo={repo}, discipline={discipline}"
            )
        else:
        ...
        if has_filters:
            try:
                ...
                if cache_data:
                    ...
                    _node_metrics_data = load_chunked(
                        metrics_collection, "network_metrics"
                    )
                    _node_metrics = (
                        _node_metrics_data.get("node_metrics", {})
                        if _node_metrics_data
                        else {}
                    )
            ...
            if not _node_metrics:
                try:
                    ...
                    if _disc_doc and _disc_doc.get("mapping"):
                        ...
                        logger.info(
                            f"📋 Fallback: cargado user_disciplines con {len(_login_to_discipline)} usuarios"
                        )
                except Exception as disc_err:
                    logger.warning(
                        f"⚠️ No se pudo cargar user_disciplines fallback: {disc_err}"
                    )
        ...
        if discipline and (_node_metrics or _login_to_discipline):
            ...
            if _node_metrics:
                for node_id, m in _node_metrics.items():
                    if (
                        node_id.startswith("user_")
                        and m.get("discipline") == discipline
                    ):
            ...
            logger.info(
                f"🔬 Filtro disciplina '{discipline}': {len(discipline_logins)} usuarios encontrados"
            )
        ...
        if org:
            repo_filter["$or"] = [{"owner.login": org}, {"organization.login": org}]
        ...
        total_repos = (
            repos_collection.count_documents(repo_filter, maxTimeMS=10000)
            if repo_filter
            else repos_collection.count_documents({}, maxTimeMS=10000)
        )
        ...
        if org or language:
            ...
            users_count_result = list(
                repos_collection.aggregate(users_pipeline, maxTimeMS=15000)
            )
        ...
        if org:
            ...
            orgs_count_result = list(
                repos_collection.aggregate(orgs_pipeline, maxTimeMS=15000)
            )
        ...
        avg_stars_pipeline.append(
            {"$group": {"_id": None, "avgStars": {"$avg": "$stargazer_count"}}}
        )
        avg_stars_result = list(repos_collection.aggregate(avg_stars_pipeline))
        avg_stars = _rounded_aggregate_value(avg_stars_result, "avgStars")
        ...
        avg_expertise_pipeline = [
            {"$match": {"quantum_expertise_score": {"$exists": True, "$ne": None}}},
            {
                "$group": {
                    "_id": None,
                    "avgExpertise": {"$avg": "$quantum_expertise_score"},
                }
            },
        ]
        avg_expertise_result = list(users_collection.aggregate(avg_expertise_pipeline))
        avg_expertise = _rounded_aggregate_value(avg_expertise_result, "avgExpertise")
        ...
        if org:
            ...
            lang_match["$or"] = [{"owner.login": org}, {"organization.login": org}]
        ...
        top_language = (
            language_distribution[0]["name"] if language_distribution else "N/A"
        )
        ...
        try:
            for col_name in ["repositories", "users", "organizations"]:
                ...
                last_doc = col.find_one(
                    {"ingested_at": {"$exists": True}}, sort=[("ingested_at", -1)]
                )
        ...
        if language:
            ...
            lang_org_base = [
                ...
                },
                {
                    "$project": {
                        ...
                        "members_count": {
                            "$ifNull": [
                                {"$arrayElemAt": ["$org_info.members_count", 0]},
                                0,
                            ]
                        },
                        ...
                        "quantum_focus_score": {
                            "$ifNull": [
                                {"$arrayElemAt": ["$org_info.quantum_focus_score", 0]},
                                0,
                            ]
                        },
                        ...
                        "is_verified": {
                            "$ifNull": [
                                {"$arrayElemAt": ["$org_info.is_verified", 0]},
                                False,
                            ]
                        },
                        ...
                        "twitter_username": {
                            "$arrayElemAt": ["$org_info.twitter_username", 0]
                        },
                        ...
                        "quantum_contributors_count": {
                            "$ifNull": [
                                {
                                    "$arrayElemAt": [
                                        "$org_info.quantum_contributors_count",
                                        0,
                                    ]
                                },
                                0,
                        ...
                        "total_repositories_count": {
                            "$ifNull": [
                                {
                                    "$arrayElemAt": [
                                        "$org_info.total_repositories_count",
                                        0,
                                    ]
                                },
                                0,
                        ...
                        "total_members_count": {
                            "$ifNull": [
                                {"$arrayElemAt": ["$org_info.total_members_count", 0]},
                                0,
                            ]
                        },
                        "total_unique_contributors": {
                            "$ifNull": [
                                {
                                    "$arrayElemAt": [
                                        "$org_info.total_unique_contributors",
                                        0,
                                    ]
                                },
                                0,
                        ...
                        "top_languages": {
                            "$ifNull": [
                                {"$arrayElemAt": ["$org_info.top_languages", 0]},
                                [],
                            ]
                        },
                        "is_quantum_focused": {
                            "$ifNull": [
                                {"$arrayElemAt": ["$org_info.is_quantum_focused", 0]},
                                False,
                            ]
                        },
                            ...
                            "$slice": [
                                {
                                    "$ifNull": [
                                        {
                                            "$arrayElemAt": [
                                                "$org_info.top_quantum_contributors",
                                                0,
                                            ]
                                        },
                                        [],
            ...
            chart_orgs = {
                ...
                "byStars": list(
                    repos_collection.aggregate(
                        lang_org_base + [{"$sort": {"total_stars": -1}}, {"$limit": 10}]
                    )
                ),
                "byQuantumFocus": list(
                    repos_collection.aggregate(
            ...
            org_base_projection = {
                ...
                "quantum_repositories_count": {
                    "$ifNull": ["$quantum_repositories_count", 0]
                },
                ...
                "email": 1,
                "quantum_contributors_count": {
                    "$ifNull": ["$quantum_contributors_count", 0]
                },
                "total_repositories_count": {
                    "$ifNull": ["$total_repositories_count", 0]
                },
                "total_members_count": {"$ifNull": ["$total_members_count", 0]},
                "total_unique_contributors": {
                    "$ifNull": ["$total_unique_contributors", 0]
                },
                "top_languages": {"$ifNull": ["$top_languages", []]},
                "is_quantum_focused": {"$ifNull": ["$is_quantum_focused", False]},
                "top_quantum_contributors": {
                    "$ifNull": [{"$slice": ["$top_quantum_contributors", 5]}, []]
                },
            }
            ...
            chart_orgs = {
                "byRepos": list(
                    orgs_collection.aggregate(
                        make_org_pipeline("quantum_repositories_count")
                    )
                ),
                "byStars": list(
                    orgs_collection.aggregate(make_org_pipeline("total_stars"))
                ),
                "byQuantumFocus": list(
                    orgs_collection.aggregate(
                        make_org_pipeline(
                ...
                "byContributors": list(
                    orgs_collection.aggregate(
                        make_org_pipeline("total_unique_contributors")
                    )
                ),
            }
        ...
        if language and repo_filter:
            ...
            if "$and" in repo_filter:
                ...
                for clause in repo_filter["$and"]:
                    if "$or" in clause and any(
                        "primary_language" in str(cond) for cond in clause["$or"]
                    ):
                        lang_part = clause
                        break
            elif "$or" in repo_filter and any(
                "primary_language" in str(cond) for cond in repo_filter["$or"]
            ):
        ...
        shared_users_org_pipeline = [
            ...
            {"$unwind": "$collaborators"},
            {
                "$group": {
                    ...
                    "orgs": {
                        "$addToSet": {
                            "$ifNull": ["$owner.login", "$organization.login"]
                        }
                    },
                }
            },
            {"$match": {"_id": {"$ne": None}}},
            {"$project": {"orgs": 1, "orgs_count": {"$size": "$orgs"}}},
            ...
            org_info_docs = {
                ...
                for d in orgs_collection.find(
                    {"login": {"$in": top_shared_logins}},
                    {
                        ...
                        "twitter_username": 1,
                        "email": 1,
                        "quantum_contributors_count": 1,
                        "total_repositories_count": 1,
                        "total_members_count": 1,
                        "total_unique_contributors": 1,
                        "top_languages": 1,
                        "is_quantum_focused": 1,
                        "top_quantum_contributors": 1,
                    },
                )
            }
            by_shared = []
                ...
                by_shared.append(
                    {
                        ...
                        "quantum_repositories_count": info.get(
                            "quantum_repositories_count", 0
                        ),
        ...
        repo_base_projection = {
            ...
            "merged_pull_requests_count": {
                "$ifNull": ["$merged_pull_requests_count", 0]
            },
        ...
        chart_repos_stars = list(
            repos_collection.aggregate(make_repo_pipeline("stargazer_count"))
        )

        # Top 10 por forks
        chart_repos_forks = list(
            repos_collection.aggregate(make_repo_pipeline("fork_count"))
        )

        # Top 10 por colaboradores
        chart_repos_collabs = list(
            repos_collection.aggregate(make_repo_pipeline("collaborators_count"))
        )
        ...
        shared_collabs_repo_pipeline = [
            ...
            {"$match": {"_id": {"$ne": None}}},
            {"$project": {"repos": 1, "repos_count": {"$size": "$repos"}}},
            {"$match": {"repos_count": {"$gte": 2}}},
            {"$unwind": "$repos"},
            {"$group": {"_id": "$repos", "shared_collaborators_count": {"$sum": 1}}},
            {"$match": {"_id": {"$ne": None}}},
            ...
            },
            {
                "$project": {
                    ...
                    "stargazer_count": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$repo_info.stargazer_count", 0]},
                            0,
                        ]
                    },
                    "fork_count": {
                        "$ifNull": [{"$arrayElemAt": ["$repo_info.fork_count", 0]}, 0]
                    },
                    "collaborators_count": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$repo_info.collaborators_count", 0]},
                            0,
                        ]
                    },
                    "primary_language": {
                        "$arrayElemAt": ["$repo_info.primary_language", 0]
                    },
                    ...
                    "repository_topics": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$repo_info.repository_topics", 0]},
                            [],
                        ]
                    },
                    ...
                    "commits_count": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$repo_info.commits_count", 0]},
                            0,
                        ]
                    },
                    "issues_count": {
                        "$ifNull": [{"$arrayElemAt": ["$repo_info.issues_count", 0]}, 0]
                    },
                    "open_issues_count": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$repo_info.open_issues_count", 0]},
                            0,
                        ]
                    },
                    "pull_requests_count": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$repo_info.pull_requests_count", 0]},
                            0,
                        ]
                    },
                    "merged_pull_requests_count": {
                        "$ifNull": [
                            {
                                "$arrayElemAt": [
                                    "$repo_info.merged_pull_requests_count",
                                    0,
                                ]
                            },
                            0,
                    ...
                    "open_pull_requests_count": {
                        "$ifNull": [
                            {
                                "$arrayElemAt": [
                                    "$repo_info.open_pull_requests_count",
                                    0,
                                ]
                            },
                            0,
                    ...
                    "releases_count": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$repo_info.releases_count", 0]},
                            0,
                        ]
                    },
                    "latest_release": {
                        "$arrayElemAt": ["$repo_info.latest_release", 0]
                    },
                    "license_info": {"$arrayElemAt": ["$repo_info.license_info", 0]},
                    "is_fork": {
                        "$ifNull": [{"$arrayElemAt": ["$repo_info.is_fork", 0]}, False]
                    },
                    "is_archived": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$repo_info.is_archived", 0]},
                            False,
                        ]
                    },
                    "watchers_count": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$repo_info.watchers_count", 0]},
                            0,
                        ]
                    },
                    "languages": {
                    ...
                    "default_branch_ref_name": {
                        "$arrayElemAt": ["$repo_info.default_branch_ref_name", 0]
                    },
                    "shared_collaborators_count": 1,
                }
            },
        ]
        chart_repos_shared = list(
            repos_collection.aggregate(shared_collabs_repo_pipeline)
        )
        ...
        if repo:
            ...
            if repo_doc and repo_doc.get("collaborators"):
                ...
                if not include_bots:
                    collaborators = [
                        c for c in collaborators if not is_bot(c.get("login", ""))
                    ]
                ...
                if collab_type == "contributors":
                    ...
                    collaborators = [
                        c for c in collaborators if c.get("has_commits", False)
                    ]
                    ...
                    collaborators = [
                        ...
                        if c.get("is_mentionable", False)
                        and not c.get("has_commits", False)
                    ]
                ...
                if discipline_logins is not None:
                    collaborators = [
                        c for c in collaborators if c.get("login") in discipline_logins
                    ]
                ...
                for collab in sorted(
                    collaborators, key=lambda x: x.get("contributions", 0), reverse=True
                )[:10]:
                    ...
                    chart_users.append(
                        {
                            ...
                            or (user_info.get("avatar_url") if user_info else None),
                            ...
                            "total_commit_contributions": (
                                user_info.get("total_commit_contributions", 0)
                                if user_info
                                else 0
                            ),
                            "total_pr_contributions": (
                                user_info.get("total_pr_contributions", 0)
                                if user_info
                                else 0
                            ),
                            "total_pr_review_contributions": (
                                user_info.get("total_pr_review_contributions", 0)
                                if user_info
                                else 0
                            ),
                            "total_issue_contributions": (
                                user_info.get("total_issue_contributions", 0)
                                if user_info
                                else 0
                            ),
                            "organizations": (
                                user_info.get("organizations", []) if user_info else []
                            ),
                            "bio": user_info.get("bio") if user_info else None,
                            "company": user_info.get("company") if user_info else None,
                            "location": (
                                user_info.get("location") if user_info else None
                            ),
                            "created_at": (
                                user_info.get("created_at") if user_info else None
                            ),
                            "followers_count": (
                                user_info.get("followers_count", 0) if user_info else 0
                            ),
                            "following_count": (
                                user_info.get("following_count", 0) if user_info else 0
                            ),
                            "public_repos_count": (
                                user_info.get("public_repos_count", 0)
                                if user_info
                                else 0
                            ),
                            "top_languages": (
                                user_info.get("top_languages", []) if user_info else []
                            ),
                            "quantum_expertise_score": (
                                user_info.get("quantum_expertise_score", 0)
                                if user_info
                                else 0
                            ),
                            "url": user_info.get("url") if user_info else None,
                            "website_url": (
                                user_info.get("website_url") if user_info else None
                            ),
                            "twitter_username": (
                                user_info.get("twitter_username") if user_info else None
                            ),
                            "is_hireable": (
                                user_info.get("is_hireable", False)
                                if user_info
                                else False
                            ),
                            "is_enriched": (
                                user_info.get("enrichment_status", {}).get(
                                    "is_complete", False
                                )
                                if user_info
                                else False
                            ),
                            "contributions_to_quantum_repos": collab.get(
                                "contributions", 0
                            ),
                        }
                    )
            ...
            if collab_type == "contributors":
                # Solo los que han hecho commits
                top_users_pipeline.append(
                    {"$match": {"collaborators.has_commits": True}}
                )
            elif collab_type == "reviewers":
                ...
                [
                    {
                        "$group": {
                            ...
                            "total_contributions": {
                                "$sum": {"$ifNull": ["$collaborators.contributions", 0]}
                            },
                            "has_commits": {
                                "$max": {
                                    "$ifNull": ["$collaborators.has_commits", False]
                                }
                            },
                            "is_mentionable": {
                                "$max": {
                                    "$ifNull": ["$collaborators.is_mentionable", False]
                                }
                            },
                        }
                    },
                        ...
                        "$project": {
                            ...
                            "total_commit_contributions": {
                                "$ifNull": [
                                    {
                                        "$arrayElemAt": [
                                            "$user_info.total_commit_contributions",
                                            0,
                                        ]
                                    },
                                    0,
                            ...
                            "total_pr_contributions": {
                                "$ifNull": [
                                    {
                                        "$arrayElemAt": [
                                            "$user_info.total_pr_contributions",
                                            0,
                                        ]
                                    },
                                    0,
                            ...
                            "total_pr_review_contributions": {
                                "$ifNull": [
                                    {
                                        "$arrayElemAt": [
                                            "$user_info.total_pr_review_contributions",
                                            0,
                                        ]
                                    },
                                    0,
                            ...
                            "total_issue_contributions": {
                                "$ifNull": [
                                    {
                                        "$arrayElemAt": [
                                            "$user_info.total_issue_contributions",
                                            0,
                                        ]
                                    },
                                    0,
                            ...
                            "organizations": {
                                "$ifNull": [
                                    {"$arrayElemAt": ["$user_info.organizations", 0]},
                                    [],
                                ]
                            },
                            ...
                            "created_at": {
                                "$arrayElemAt": ["$user_info.created_at", 0]
                            },
                            "followers_count": {
                                "$ifNull": [
                                    {"$arrayElemAt": ["$user_info.followers_count", 0]},
                                    0,
                                ]
                            },
                            "following_count": {
                                "$ifNull": [
                                    {"$arrayElemAt": ["$user_info.following_count", 0]},
                                    0,
                                ]
                            },
                            "public_repos_count": {
                            ...
                            "top_languages": {
                                "$ifNull": [
                                    {"$arrayElemAt": ["$user_info.top_languages", 0]},
                                    [],
                                ]
                            },
                            "quantum_expertise_score": {
                                "$ifNull": [
                                    {
                                        "$arrayElemAt": [
                                            "$user_info.quantum_expertise_score",
                                            0,
                                        ]
                                    },
                                    0,
                            ...
                            "website_url": {
                                "$arrayElemAt": ["$user_info.website_url", 0]
                            },
                            "twitter_username": {
                                "$arrayElemAt": ["$user_info.twitter_username", 0]
                            },
                            "is_hireable": {
                                "$ifNull": [
                                    {"$arrayElemAt": ["$user_info.is_hireable", 0]},
                                    False,
                                ]
                            },
                            "is_enriched": {
                                "$ifNull": [
                                    {
                                        "$arrayElemAt": [
                                            "$user_info.enrichment_status.is_complete",
                                            0,
                                        ]
                                    },
                                    False,
            ...
            if discipline_logins is not None:
                for i, stage in enumerate(top_users_pipeline):
                    if "$sort" in stage:
                        top_users_pipeline.insert(
                            i, {"$match": {"_id": {"$in": list(discipline_logins)}}}
                        )
                        break
                    ...
                    [
                        {
                            "$group": {
                                ...
                                "total_contributions": {
                                    "$sum": {
                                        "$ifNull": ["$collaborators.contributions", 0]
                                    }
                                },
                                "has_commits": {
                                    "$max": {
                        ...
                        },
                        {
                            "$project": {
                                ...
                                "company": {"$arrayElemAt": ["$user_info.company", 0]},
                                "location": {
                                    "$arrayElemAt": ["$user_info.location", 0]
                                },
                                "created_at": {
                                    "$arrayElemAt": ["$user_info.created_at", 0]
                                },
                ...
                top_users_pipeline = [
                    {"$match": user_match},
                    {
                        "$project": {
                            ...
                            "public_repos_count": {
                                "$ifNull": ["$public_repos_count", 0]
                            },
                            "top_languages": {"$ifNull": ["$top_languages", []]},
                            "quantum_expertise_score": {
                                "$ifNull": ["$quantum_expertise_score", 0]
                            },
                            "url": 1,
                            "website_url": 1,
                            "twitter_username": 1,
                            ...
                            "is_enriched": {
                                "$ifNull": ["$enrichment_status.is_complete", False]
                            },
                        }
                    },
                    # collab_score = round(sqrt(contributions × repos × 100))
        ...
        multi_repo_user_pipeline = [
            ...
            {"$unwind": "$collaborators"},
            {
                "$group": {
                    ...
                    "has_commits": {
                        "$max": {"$ifNull": ["$collaborators.has_commits", False]}
                    },
                    "is_mentionable": {
                        "$max": {"$ifNull": ["$collaborators.is_mentionable", False]}
                    },
                }
            },
            {"$match": {"_id": {"$ne": None}}},
            ...
            },
            {
                "$project": {
                    ...
                    "total_pr_contributions": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$user_info.total_pr_contributions", 0]},
                            0,
                        ]
                    },
                    "total_pr_review_contributions": {
                    ...
                    "public_repos_count": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$user_info.public_repos_count", 0]},
                            0,
                        ]
                    },
                    "top_languages": {
                    ...
                    "quantum_expertise_score": {
                        "$ifNull": [
                            {"$arrayElemAt": ["$user_info.quantum_expertise_score", 0]},
                            0,
                        ]
                    },
                    "url": {"$arrayElemAt": ["$user_info.url", 0]},
        ...
        if discipline_logins is not None:
            for i, stage in enumerate(multi_repo_user_pipeline):
                if "$sort" in stage:
                    multi_repo_user_pipeline.insert(
                        i, {"$match": {"_id": {"$in": list(discipline_logins)}}}
                    )
                    break
        # Filtrar bots si no se incluyen
        chart_users_by_repos_raw = list(
            repos_collection.aggregate(multi_repo_user_pipeline)
        )
        if not include_bots:
            chart_users_by_repos_raw = [
                u for u in chart_users_by_repos_raw if not is_bot(u.get("login", ""))
            ]
        ...
        if has_filters:
            ...
            try:
                if repo:
                    ...
                    if _rd and _rd.get("collaborators"):
                        ...
                        if discipline_logins is not None:
                            _cc = [
                                c for c in _cc if c.get("login") in discipline_logins
                            ]
                        _all_filtered_logins = {
                            c.get("login") for c in _cc if c.get("login")
                        }
                elif language:
                    ...
                    if discipline_logins is not None:
                        _pipe.append(
                            {"$match": {"_id": {"$in": list(discipline_logins)}}}
                        )
                    _all_filtered_logins = {
                        r["_id"]
                        for r in repos_collection.aggregate(_pipe)
                        if r.get("_id")
                    }
                    ...
                    _all_filtered_logins = {
                        r["login"]
                        for r in users_collection.find(_um, {"login": 1, "_id": 0})
                    }
            ...
            if _all_filtered_logins and (_node_metrics or _login_to_discipline):
                try:
                    ...
                    if _login_disc:
                        ...
                        _dist_pct = (
                            {k: round(v / _total * 100, 1) for k, v in _dist.items()}
                            if _total > 0
                            else {}
                        )
                        ...
                        if _total > 0 and len(_dist) > 1:
                            _entropy = -sum(
                                (v / _total) * math.log2(v / _total)
                                for v in _dist.values()
                                if v > 0
                            )
                            _max_entropy = math.log2(6)  # 6 discipline categories
                            _cdi = (
                                round(_entropy / _max_entropy * 100, 1)
                                if _max_entropy > 0
                                else 0
                            )
                        else:
                        ...
                        _existing_bridges = (
                            (_node_metrics_data or {})
                            .get("discipline_analysis", {})
                            .get("bridge_profiles", [])
                        )
                        _filtered_bridges = [
                            bp
                            for bp in _existing_bridges
                            if bp.get("login") in _all_filtered_logins
                        ]
                        ...
                        logger.info(
                            f"📊 Distribución de disciplinas filtrada: {_total} usuarios, CDI={_cdi}%"
                        )
                except Exception as _dd_err:
                    logger.warning(
                        f"⚠️ Error computing filtered discipline distribution: {_dd_err}"
                    )
            ...
            if not filtered_discipline_dist and _all_filtered_logins:
                ...
                logger.info(
                    f"📋 Enviando {len(_filtered_collab_logins)} logins filtrados para cómputo client-side"
                )
        ...
        response_data = {
            ...
            "tables": {"repositories": table_repos, "users": table_users},
            "filters": {"organizations": filter_orgs, "languages": filter_languages},
            "metadata": {
                "cached": False,
        ...
        if not has_filters:
            ...
            metrics_collection.update_one(
                {"type": "dashboard_stats"}, {"$set": cache_document}, upsert=True
            )

            logger.info(
                "✅ Dashboard stats COMPLETO calculado y guardado en caché permanente"
            )
        else:
...
async def discover_collaboration(
    ...
    year_from: Optional[int] = Query(
        default=None,
        description="Año inicio del rango temporal (incluido). Filtra repos por pushed_at.",
    ),
    year_to: Optional[int] = Query(
        default=None,
        description="Año fin del rango temporal (incluido). Filtra repos por pushed_at.",
    ),
):
    """
    ...
    try:
        ...
        if not force and not has_temporal_filter:
            ...
            if cached:
                logger.info(
                    f"[DISCOVER] Sirviendo grafo desde caché chunked ({cached.get('metrics', {}).get('graph_nodes', '?')} nodos)"
                )
                return Response(
                    content=orjson.dumps(cached), media_type="application/json"
                )
        ...
        def _is_bot_login(login: str) -> bool:
            ...
            return (
                "[bot]" in ll
                or ll.endswith("-bot")
                or (ll.endswith("bot") and len(ll) > 3)
                or ll.startswith("bot-")
                or ll in KNOWN_BOTS
            )
        ...
        if has_temporal_filter:
            ...
            logger.info(
                f"[DISCOVER] Filtro temporal: {total_before} → {len(all_repos)} repos (pushed_at en {year_from or '∞'}–{year_to or '∞'})"
            )
                    ...
                    user_to_repos[login].append(
                        {
                            ...
                            "language": (
                                repo.get("primary_language", {}).get("name")
                                if isinstance(repo.get("primary_language"), dict)
                                else repo.get("primary_language")
                            ),
                        }
                    )
        ...
        sorted_bridge = sorted(
            human_bridge_users.items(), key=lambda x: len(x[1]), reverse=True
        )
            ...
            if full_name in connected_repos:
                ...
                if node_id not in added_nodes:
                    ...
                    if isinstance(_pushed, datetime):
                        ...
                        try:
                            _pyr = datetime.fromisoformat(
                                _pushed.replace("Z", "+00:00")
                            ).year
                        except (ValueError, TypeError):
                            pass
                    nodes.append(
        ...
        def _make_user_node(login, repos_list, is_bridge):
            ...
            connected_orgs = list(
                set(r.get("owner") for r in (repos_list or []) if r.get("owner"))
            )
            return {
                "id": f"user_{login}",
                "type": "user",
                ...
                "isBot": user_is_bot,
                "quantum_expertise_score": (
                    user_info.get("quantum_expertise_score", 0) if user_info else 0
                ),
            }
            ...
            enriched_bridge_list.append(
                {
                    ...
                    "quantum_expertise_score": user_node.get(
                        "quantum_expertise_score", 0
                    ),
                ...
                nodes.append(
                    {
                        ...
                        "is_verified": (
                            org_doc.get("is_verified", False) if org_doc else False
                        ),
                        "created_at": org_doc.get("created_at") if org_doc else None,
                        "quantum_focus_score": (
                            org_doc.get("quantum_focus_score", 0) if org_doc else 0
                        ),
                        "is_quantum_focused": (
                            org_doc.get("is_quantum_focused", False)
                            if org_doc
                            else False
                        ),
                        "quantum_repos_count": (
                            org_doc.get("quantum_repositories_count", 0)
                            if org_doc
                            else 0
                        ),
                        "total_repos_count": (
                            org_doc.get("total_repositories_count")
                            or org_doc.get("public_repos_count", 0)
                            if org_doc
                            else 0
                        ),
                        "members_count": (
                            org_doc.get("total_members_count")
                            or org_doc.get("members_count", 0)
                            if org_doc
                            else 0
                        ),
                        "total_stars": org_doc.get("total_stars", 0) if org_doc else 0,
                        "top_languages": (
                            (org_doc.get("top_languages") or [])[:5] if org_doc else []
                        ),
                        # Graph-level stats (computed from current collaboration data)
                        "graph_repos_count": _org_repo_counts.get(org_login, 0),
                        "graph_contributors_count": len(
                            _org_contrib_logins.get(org_login, set())
                        ),
                        "graph_bridge_count": len(
                            _org_bridge_logins.get(org_login, set())
                        ),
                    }
                )
                ...
                for repo in all_repos:
                    if repo.get("owner", {}).get("login") == org_login:
                        ...
                        if repo_id in added_nodes:
                            links.append(
                                {"source": org_id, "target": repo_id, "type": "owns"}
                            )
        ...
        if sibling_pairs_skipped > 0:
            logger.info(
                f"[DISCOVER] Sibling org pairs excluidos: {sibling_pairs_skipped}"
            )
        logger.info(
            f"[DISCOVER] Entrelazamientos org↔org genuinos: {entanglement_count} (threshold ≥3 bridge users)"
        )
        ...
        if len(cross_org_users) > 0:
            summary_parts.append(
                f"{len(cross_org_users)} bridge users (cross-org) entre {len(connected_repos)} repositorios"
            )
        if len(connected_repo_pairs) > 0:
            summary_parts.append(
                f"{len(connected_repo_pairs)} pares de repos conectados"
            )
        if len(cross_org_users) > 0:
        ...
        repo_nodes_years = [
            n.get("pushed_at_year")
            for n in nodes
            if n.get("type") == "repo" and n.get("pushed_at_year")
        ]
        temporal_range = None
        if repo_nodes_years:
            temporal_range = {
                "min": min(repo_nodes_years),
                "max": max(repo_nodes_years),
            }
        ...
        if not has_temporal_filter:
            try:
                ...
                logger.info(
                    f"[DISCOVER] Grafo guardado en caché chunked ({len(nodes)} nodos, {len(links)} links)"
                )
                ...
                cross_org_bridge_list.sort(
                    key=lambda x: (x.get("orgs_count", 0), x.get("repos_count", 0)),
                    reverse=True,
                )
                ...
                logger.info(
                    f"[DISCOVER] Resumen cross-org bridges guardado ({len(cross_org_bridge_list)} usuarios)"
                )
            except Exception as cache_err:
                logger.warning(f"[DISCOVER] No se pudo cachear el grafo: {cache_err}")
        else:
            logger.info(
                f"[DISCOVER] Filtro temporal activo → caché omitida ({len(nodes)} nodos, {len(links)} links)"
            )
...
async def invalidate_collaboration_cache():
    ...
    try:
        ...
        logger.info(
            f"[DISCOVER] Caché invalidada: {'eliminada' if deleted else 'no existía'} ({deleted_count} docs)"
        )
        return {
            "invalidated": deleted,
            "message": (
                f"Caché del grafo eliminada ({deleted_count} docs)"
                if deleted
                else "No había caché"
            ),
        }
...
async def analyze_collaboration(
    repos: Optional[list] = Query(
        default=None, description="Lista de repositorios (full_name) a analizar"
    ),  # noqa: B008
    orgs: Optional[list] = Query(
        default=None, description="Lista de organizaciones (login) a analizar"
    ),  # noqa: B008
    user: Optional[str] = Query(
        default=None, description="Usuario específico para ver sus colaboraciones"
    ),
):
    """
    ...
    try:
        ...
        if user:
            ...
            if not user_doc:
                raise HTTPException(
                    status_code=404, detail=f"Usuario {user} no encontrado"
                )
            ...
            for repo in user_repos:
                for collab in repo.get("collaborators") or []:  # pragma: no cover
                    ...
                    if login and login != user:
                        ...
                        co_collaborators[login][
                            "total_shared_contributions"
                        ] += collab.get("contributions", 0)
            ...
            for collab in sorted_collaborators:
                ...
                if user_info:
                    ...
                    collab["quantum_expertise_score"] = user_info.get(
                        "quantum_expertise_score", 0
                    )
                    ...
                    nodes.append(
                        {
                            ...
                            "login": org_login,
                            "name": (
                                org.get("name") if isinstance(org, dict) else org_login
                            ),
                        }
                    )
                    links.append(
        ...
        if repos and len(repos) >= 2:
            ...
            for repo_name in repos:
                ...
                if repo_doc and repo_doc.get("collaborators"):
                    collabs = {
                        c.get("login")
                        for c in (repo_doc.get("collaborators") or [])
                        if c.get("login")
                    }  # pragma: no cover
            ...
            shared_users = []
            for login, repos_list in sorted(
                user_appearances.items(), key=lambda x: len(x[1]), reverse=True
            )[:20]:
                ...
                shared_users.append(
                    {
                        ...
                        "name": user_info.get("name") if user_info else None,
                        "avatar_url": (
                            user_info.get("avatar_url") if user_info else None
                        ),
                        "quantum_expertise_score": (
                            user_info.get("quantum_expertise_score", 0)
                            if user_info
            ...
            result["metrics"] = {
                ...
                "collaboration_density": round(
                    len(user_appearances) / max(len(all_users), 1) * 100, 2
                ),
            }
        ...
        if orgs and len(orgs) >= 2:
            ...
            for user_login in all_users:
                orgs_with_user = [
                    org_login
                    for org_login, users in org_users.items()
...
async def get_network_metrics(
    ...
    year_from: Optional[int] = Query(
        default=None, description="Año inicio del rango temporal (incluido)"
    ),
    year_to: Optional[int] = Query(
        default=None, description="Año fin del rango temporal (incluido)"
    ),
):
    """
    ...
    try:
        ...
        if (
            not force_refresh
            and not has_temporal_filter
            and cache["json_bytes"]
            and cache["computed_at"]
        ):
            logger.info(
                "[NetworkMetrics] Devolviendo desde caché en memoria (permanente)"
            )
        ...
        if not force_refresh and not has_temporal_filter:
            ...
            if cached_result:
                ...
                logger.info(
                    f"[NetworkMetrics] Restaurado desde caché MongoDB chunked permanente ({len(json_bytes) / 1024:.0f} KB)"
                )
        ...
        if has_temporal_filter:
            logger.info(
                f"[NetworkMetrics] Construyendo grafo CON FILTRO TEMPORAL ({year_from or '∞'}–{year_to or '∞'})..."
            )
        else:
        ...
        analyzer.build_from_mongodb(
            repos_col, users_col, orgs_col, year_from=year_from, year_to=year_to
        )
        full = analyzer.get_full_analysis(
            users_collection=users_col, repos_collection=repos_col
        )
        ...
        if not has_temporal_filter:
            try:
                ...
                logger.warning(
                    f"[NetworkMetrics] No se pudo persistir a MongoDB: {mongo_err}"
                )
            ...
            try:
                ...
                if _disc_mapping:
                    metrics_collection.replace_one(
                        ...
                        {
                            "_id": "user_disciplines",
                            "mapping": _disc_mapping,
                            "_cached_at": datetime.now(timezone.utc),
                        },
                        upsert=True,
                    )
                    logger.info(
                        f"[NetworkMetrics] Guardado mapeo user_disciplines: {len(_disc_mapping)} usuarios"
                    )
            except Exception as disc_err:
                logger.warning(
                    f"[NetworkMetrics] No se pudo guardar user_disciplines: {disc_err}"
                )

        cache_label = (
            "sin cachear (filtro temporal)"
            if has_temporal_filter
            else "cacheado en memoria + MongoDB"
        )
        logger.info(
...
async def quantum_tunneling(
    ...
    target: str = Query(
        ..., description="ID del nodo destino (ej: repo_qiskit/qiskit)"
    ),
):
    """
    ...
    try:
        ...
        if not analyzer:
            ...
            if cached_nm:
                ...
                logger.info(
                    "[QuantumTunneling] Reconstruyendo analyzer desde caché MongoDB..."
                )
                repos_col = db.get_collection("repositories")
                users_col = db.get_collection("users")
                orgs_col = db.get_collection("organizations")
                ...
                _network_metrics_cache["analyzer"] = analyzer
                logger.info(
                    "[QuantumTunneling] Analyzer reconstruido y cacheado en memoria"
                )
            else:
                logger.info(
                    "[QuantumTunneling] Sin caché, construyendo grafo completo..."
                )
                db.ensure_connection()
                repos_col = db.get_collection("repositories")
                users_col = db.get_collection("users")
...
async def list_repositories(
    ...
    limit: int = Query(
        default=0, ge=0, description="Límite de resultados (0 = sin límite)"
    ),
...
async def get_user_profile(login: str):
    ...
    try:
        ...
        user["total_contributions"] = (
            github_contribs if github_contribs > 0 else repo_contribs
        )
        user["contributions_to_quantum_repos"] = repo_contribs
        user["is_enriched"] = user.get("enrichment_status", {}).get(
            "is_complete", False
        )
...
async def ingest_repositories(
    ...
    max_results: Optional[int] = Query(
        None, description="Máximo de repositorios a ingerir"
    ),
    incremental: bool = Query(
        False,
        description="Modo incremental (solo repos nuevos/actualizados desde última ingesta)",
    ),
    from_scratch: bool = Query(
        False,
        description="Modo desde cero (limpia colección y reingesta todo, elimina datos zombi)",
    ),
    use_segmentation: bool = Query(
        True, description="Usar segmentación dinámica para más de 1000 repos"
    ),
    max_workers: int = Query(
        4, description="Workers paralelos para búsquedas segmentadas (1-8)"
    ),
):
    """
...
async def ingest_users(
    background_tasks: BackgroundTasks,
    max_repos: Optional[int] = Query(
        None, description="Máximo de repositorios a procesar"
    ),
    batch_size: int = Query(50, description="Tamaño del lote para procesamiento"),
    from_scratch: bool = Query(
        False, description="Modo desde cero (limpia colección de usuarios y reingesta)"
    ),
):
    """
    ...
    try:
        ...
        background_tasks.add_task(
            _run_user_ingestion, task_id, max_repos, batch_size, from_scratch
        )
...
async def enrich_repositories(
    background_tasks: BackgroundTasks,
    max_repos: Optional[int] = Query(
        None, description="Máximo de repositorios a enriquecer"
    ),
    force_reenrich: bool = Query(
        False, description="Re-enriquecer incluso repositorios ya enriquecidos"
    ),
    ...
    try:
        ...
        background_tasks.add_task(
            _run_repository_enrichment, task_id, max_repos, force_reenrich, batch_size
        )
...
async def enrich_users(
    ...
    max_users: Optional[int] = Query(
        None, description="Máximo de usuarios a enriquecer"
    ),
    force_reenrich: bool = Query(
        False, description="Re-enriquecer incluso usuarios ya enriquecidos"
    ),
    batch_size: int = Query(10, description="Tamaño del lote para procesamiento"),
):
    """
    ...
    try:
        ...
        background_tasks.add_task(
            _run_user_enrichment, task_id, max_users, force_reenrich, batch_size
        )
...
async def ingest_organizations(
    ...
    force_update: bool = Query(
        False, description="Actualizar organizaciones ya existentes"
    ),
    batch_size: int = Query(5, description="Tamaño del lote para procesamiento"),
    from_scratch: bool = Query(
        False,
        description="Modo desde cero (limpia colección de organizaciones y reingesta)",
    ),
):
    """
    ...
    try:
        ...
        background_tasks.add_task(
            _run_organization_ingestion, task_id, force_update, batch_size, from_scratch
        )
...
async def enrich_organizations(
    background_tasks: BackgroundTasks,
    max_orgs: Optional[int] = Query(
        None, description="Máximo de organizaciones a enriquecer"
    ),
    force_reenrich: bool = Query(
        False, description="Re-enriquecer incluso organizaciones ya enriquecidas"
    ),
    batch_size: int = Query(5, description="Tamaño del lote para procesamiento"),
):
    """
    ...
    try:
        ...
        background_tasks.add_task(
            _run_organization_enrichment, task_id, max_orgs, force_reenrich, batch_size
        )

        logger.info(
            f"✅ Tarea de enriquecimiento de organizaciones iniciada: {task_id}"
        )
...
def _run_repository_ingestion(
    ...
    try:
        ...
        background_tasks_status[task_id][
            "progress"
        ] = f"Creando motor de ingesta (modo: {mode})..."

        # 1. Creamos el motor con soporte de modos
        engine = IngestionEngine(
            incremental=incremental, from_scratch=from_scratch, max_workers=max_workers
        )
        ...
        background_tasks_status[task_id][
            "progress"
        ] = f"Ejecutando ingesta (modo: {mode})..."
        ...
        try:
            ...
            logger.warning(
                f"✅ Tarea {task_id} completada (error al invalidar cachés: {cache_err})"
            )
...
def _run_user_ingestion(
    task_id: str, max_repos: Optional[int], batch_size: int, from_scratch: bool = False
):
    ...
    try:
        ...
        background_tasks_status[task_id][
            "progress"
        ] = f"Creando motor de ingesta de usuarios (modo: {mode})..."
        ...
        background_tasks_status[task_id][
            "progress"
        ] = f"Ejecutando ingesta de usuarios (modo: {mode})..."
...
def _run_repository_enrichment(
    task_id: str, max_repos: Optional[int], force_reenrich: bool, batch_size: int
):
    """Ejecuta el enriquecimiento de repositorios en background."""
    try:
        background_tasks_status[task_id][
            "progress"
        ] = "Creando motor de enriquecimiento..."
        ...
        background_tasks_status[task_id][
            "progress"
        ] = "Ejecutando enriquecimiento de repositorios..."

        stats = engine.enrich_all_repositories(
            max_repos=max_repos, force_reenrich=force_reenrich
        )
...
def _run_user_enrichment(
    task_id: str, max_users: Optional[int], force_reenrich: bool, batch_size: int
):
    """Ejecuta el enriquecimiento de usuarios en background."""
    try:
        background_tasks_status[task_id][
            "progress"
        ] = "Creando motor de enriquecimiento de usuarios..."
        ...
        stats = engine.enrich_all_users(
            max_users=max_users, force_reenrich=force_reenrich
        )

        background_tasks_status[task_id]["status"] = "completed"
        background_tasks_status[task_id][
            "progress"
        ] = "Enriquecimiento de usuarios completado"
        background_tasks_status[task_id]["stats"] = stats
        background_tasks_status[task_id]["completed_at"] = datetime.now().isoformat()
...
def _run_organization_ingestion(
    task_id: str, force_update: bool, batch_size: int, from_scratch: bool = False
):
    ...
    try:
        ...
        background_tasks_status[task_id][
            "progress"
        ] = f"Creando motor de ingesta de organizaciones (modo: {mode})..."
        ...
        background_tasks_status[task_id][
            "progress"
        ] = f"Ejecutando ingesta de organizaciones (modo: {mode})..."
        ...
        background_tasks_status[task_id][
            "progress"
        ] = "Ingesta de organizaciones completada"
        background_tasks_status[task_id]["stats"] = stats
        background_tasks_status[task_id]["completed_at"] = datetime.now().isoformat()
...
def _run_organization_enrichment(
    task_id: str, max_orgs: Optional[int], force_reenrich: bool, batch_size: int
):
    """Ejecuta el enriquecimiento de organizaciones en background."""
    try:
        background_tasks_status[task_id][
            "progress"
        ] = "Creando motor de enriquecimiento de organizaciones..."
        ...
        stats = engine.enrich_all_organizations(
            max_orgs=max_orgs, force_reenrich=force_reenrich
        )

        background_tasks_status[task_id]["status"] = "completed"
        background_tasks_status[task_id][
            "progress"
        ] = "Enriquecimiento de organizaciones completado"
        background_tasks_status[task_id]["stats"] = stats
        background_tasks_status[task_id]["completed_at"] = datetime.now().isoformat()
...
async def run_full_pipeline(
    ...
    mode: str = Query(
        "incremental",
        description="Modo de ingesta: 'incremental' (solo datos nuevos) o 'from_scratch' (limpia todo y reingesta)",
    ),
    max_workers: int = Query(
        4,
        ge=1,
        le=8,
        description="Workers paralelos para búsqueda segmentada de repositorios",
    ),
):
    """
    ...
    if mode not in ("incremental", "from_scratch"):
        raise HTTPException(
            status_code=400, detail="mode debe ser 'incremental' o 'from_scratch'"
        )
    ...
    background_tasks.add_task(
        _run_full_pipeline_direct, task_id, from_scratch, max_workers
    )
...
def _run_full_pipeline_direct(
    task_id: str, from_scratch: bool = False, max_workers: int = 4
):
    ...
    def run_operation(name: str, func, *args, **kwargs) -> OperationResult:
        ...
        try:
            ...
            if isinstance(result, dict):
                records_processed = (
                    result.get("total", 0)
                    or result.get("total_processed", 0)
                    or result.get("stats", {}).get("total_found", 0)
                    or result.get("total_organizations", 0)
                    or result.get("total_users", 0)
                    or result.get("enriched", 0)
                    or result.get("new_organizations", 0)
                    or result.get("new_users", 0)
                    or result.get("users_inserted", 0)
                    or 0
                )
    ...
    try:
        ...
        background_tasks_status[task_id][
            "progress"
        ] = f"1/6 - Ingesta de Repositorios ({mode_label})"
        ...
        background_tasks_status[task_id][
            "progress"
        ] = "2/6 - Enriquecimiento de Repositorios"
        ...
        background_tasks_status[task_id][
            "progress"
        ] = f"3/6 - Ingesta de Usuarios ({mode_label})"
        result = run_operation(
            "3. Ingesta de Usuarios",
        ...
        background_tasks_status[task_id][
            "progress"
        ] = "4/6 - Enriquecimiento de Usuarios"
        users_repo = MongoRepository("users")
        ...
        background_tasks_status[task_id][
            "progress"
        ] = f"5/6 - Ingesta de Organizaciones ({mode_label})"
        orgs_repo = MongoRepository("organizations")
        ...
        result = run_operation(
            ...
            lambda: OrganizationIngestionEngine(
                ...
                force_update=from_scratch
            ),  # force_update=True cuando es desde cero
        )
        results.append(result)
        ...
        background_tasks_status[task_id][
            "progress"
        ] = "6/6 - Enriquecimiento de Organizaciones"
        result = run_operation(
            "6. Enriquecimiento de Organizaciones",
        ...
        for result in results:
            ...
            logger.info(
                f"{result.name}: {status} ({result.records_processed:,} registros, {result.duration:.0f}s)"
            )
        ...
        if successful == len(results):
            ...
            background_tasks_status[task_id][
                "progress"
            ] = f"Completado con {len(results) - successful} errores"
...
def _run_full_pipeline_script(task_id: str):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            background_tasks_status[task_id][
                "progress"
            ] = f"Pipeline fallo con codigo {result.returncode}"
        ...
        logger.info(
            f"Pipeline completado - Task ID: {task_id}, Exit Code: {result.returncode}"
        )
        ...
        try:
            ...
            logger.warning(
                f"⚠️ Error al invalidar cachés tras pipeline script: {cache_err}"
            )
...
async def add_favorite(favorite: Dict[str, Any]):
    ...
    try:
        ...
        prefs.update_one(
            {"type": "favorites"},
            {"$push": {"items": item}, "$set": {"updated_at": datetime.now()}},
        )
...
async def save_view(view: Dict[str, Any]):
    ...
    try:
        ...
        if "name" not in view or "entity_ids" not in view:
            raise HTTPException(
                status_code=400, detail="Campos requeridos: name, entity_ids"
            )

        if not isinstance(view["entity_ids"], list) or len(view["entity_ids"]) == 0:
            raise HTTPException(
                status_code=400, detail="entity_ids debe ser un array no vacío"
            )
        ...
        prefs.update_one(
            {"type": "custom_views"}, {"$pull": {"items": {"id": view_id}}}, upsert=True
        )
        prefs.update_one(
            {"type": "custom_views"},
            {"$push": {"items": item}, "$set": {"updated_at": datetime.now()}},
        ...
        logger.info(
            f"Vista guardada: '{view['name']}' con {len(view['entity_ids'])} entidades"
        )
...
async def get_view_data(view_id: str, body: Dict[str, Any] = None):
    ...
    try:
        ...
        if body and "entity_ids" in body:
            ...
            if doc:
                view_item = next(
                    (v for v in doc.get("items", []) if v["id"] == view_id), None
                )
                if view_item:
        ...
        if not entity_ids:
            raise HTTPException(
                status_code=404, detail="Vista no encontrada o sin entidades"
            )
        ...
        if repo_ids:
            collab_repos = list(
                repos_col.find({"full_name": {"$in": repo_ids}}, {"collaborators": 1})
            )
        ...
        avg_stars = sum((r.get("stargazer_count") or 0) for r in repos) / max(
            total_repos, 1
        )
        avg_expertise = sum(
            (u.get("quantum_expertise_score") or 0) for u in users
        ) / max(total_users, 1)
        ...
        lang_distribution = [
            {"name": k, "value": v}
            for k, v in sorted(lang_counts.items(), key=lambda x: -x[1])[:10]
        ]
        ...
        for org in orgs:
            ...
            org_repos_list = [
                r
                for r in repos
                if (r.get("owner", {}) or {}).get("login") == org_login
                or (r.get("organization", {}) or {}).get("login") == org_login
            ]
            chart_orgs_list.append(
                {
                    ...
                    "total_stars": sum(
                        (r.get("stargazer_count") or 0) for r in org_repos_list
                    ),
                    ...
                    "email": org.get("email"),
                    "quantum_contributors_count": org.get(
                        "quantum_contributors_count", 0
                    ),
                    "total_repositories_count": org.get("total_repositories_count")
                    or org.get("public_repos", 0),
                    "total_members_count": org.get("total_members_count")
                    or org.get("members_count", 0),
                    "total_unique_contributors": org.get(
                        "total_unique_contributors", 0
                    ),
                    "top_languages": org.get("top_languages", []),
                    "is_quantum_focused": org.get("is_quantum_focused", False),
                    "top_quantum_contributors": (
                        org.get("top_quantum_contributors") or []
                    )[:5],
                }
            )
        chart_orgs_list.sort(
            key=lambda o: o["quantum_repositories_count"], reverse=True
        )
        ...
        chart_orgs = {
            "byRepos": sorted(
                chart_orgs_list,
                key=lambda o: o["quantum_repositories_count"],
                reverse=True,
            )[:10],
            "byStars": sorted(
                chart_orgs_list, key=lambda o: o["total_stars"], reverse=True
            )[:10],
            "byQuantumFocus": sorted(
                [
                    o
                    for o in chart_orgs_list
                    if o["quantum_repositories_count"] >= MIN_REPOS_FOCUS
                ],
                key=lambda o: o.get("quantum_focus_score", 0),
                reverse=True,
            )[:10],
            "byContributors": sorted(
                chart_orgs_list,
                key=lambda o: o.get("total_unique_contributors", 0),
                reverse=True,
            )[:10],
        }
        ...
        for u in users:
            login = u.get("login", "")
            contrib_info = user_contrib_map.get(
                login, {"contributions": 0, "repos": set()}
            )
            chart_users.append(
                {
                    "login": login,
        ...
        response = {
            ...
            "charts": {
                "organizations": chart_orgs,
                "repositories": sorted(
                    repos, key=lambda r: (r.get("stargazer_count") or 0), reverse=True
                ),
                "users": chart_users,
            ...
            "tables": {
                ...
                "users": sorted(
                    chart_users,
                    key=lambda u: u.get("quantum_expertise_score", 0),
                    reverse=True,
                )[:20],
            },
            "metadata": {
        ...
        try:
            ...
            metrics_collection.update_one(
                {"_id": cache_key}, {"$set": cache_doc}, upsert=True
            )
        ...
        if entity_id.startswith("org_"):
            ...
            result = {
                ...
                "children": sorted(
                    children,
                    key=lambda x: (
                        int(x["subtitle"].split("⭐ ")[1].split(" ·")[0])
                        if "⭐" in x["subtitle"]
                        else 0
                    ),
                    reverse=True,
            ...
            repo = repos_col.find_one(
                {"full_name": repo_full_name}, {"collaborators": 1, "full_name": 1}
            )
            if not repo:
            ...
            children.sort(
                key=lambda x: (0 if x["is_bridge"] else 1, -x["contributions"])
            )
...
async def get_entity_detail(entity_id: str):
    ...
    try:
        ...
        if entity_id.startswith("user_"):
            ...
            if not doc:
                raise HTTPException(
                    status_code=404, detail=f"Usuario {login} no encontrado"
                )
            doc["_id"] = str(doc["_id"])
            ...
            pipeline = [
                ...
                {
                    "$addFields": {
                        "user_contributions": {"$sum": "$contrib.contributions"}
                    }
                },
            ]
            ...
            collab_score = round(
                math.sqrt(total_quantum_contributions * (relevant_repos_count * 100))
            )
            ...
            if not doc:
                raise HTTPException(
                    status_code=404, detail=f"Repositorio {full_name} no encontrado"
                )
            doc["_id"] = str(doc["_id"])
            ...
            if not doc:
                raise HTTPException(
                    status_code=404, detail=f"Organización {login} no encontrada"
                )
            doc["_id"] = str(doc["_id"])
            ...
            raise HTTPException(
                status_code=400, detail=f"Formato de ID no válido: {entity_id}"
            )
...
async def search_entities(
    ...
    try:
        ...
        results.sort(
            key=lambda x: (
                0
                if x["name"].lower() == q_lower or x.get("login", "").lower() == q_lower
                else (
                    1
                    if x["name"].lower().startswith(q_lower)
                    or x.get("login", "").lower().startswith(q_lower)
                    else 2
                )
            )
        )
